...
func TestDelegatedConfigParsesIssuers(t *testing.T) {
	...
	// The M1×M2 seam is CLOSED: a delegated session reports the display name,
	// the read-only phase and the suspension that the accounts API maintains
	// (contract §2.3), read from the store.
	//
	// This assertion was the inverse until the two epics merged — it required
	// Accounts to be nil, precisely so that closing the seam could not happen
	// silently. It is kept, inverted, for the same reason in the other
	// direction: removing the source would now be a visible, deliberate act,
	// and an unnoticed removal would make every delegated session report
	// readOnly = false — a read-only mailbox that offers its user a compose
	// button, which is the failure §2.4 exists to prevent.
	if cfg.Accounts == nil {
		t.Fatal("Accounts is nil; the delegated session would report no read-only or suspended fact")
	}
	if _, ok := cfg.Accounts.(storeAccountStatus); !ok {
		t.Errorf("Accounts = %T, want storeAccountStatus (the store-backed seam)", cfg.Accounts)
	}
}
