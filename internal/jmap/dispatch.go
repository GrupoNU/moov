...
//
// Most methods produce exactly one response and register a Handler. The
// exception RFC 8620 anticipates — a method whose spec mandates FURTHER
// responses under the same call id — registers a MultiHandler instead: W3's
// EmailSubmission/set is one (RFC 8621 §7.5: the onSuccess properties trigger
// "an implicit call to Email/set" whose response "MUST be returned after the
// EmailSubmission/set response"), and §5.4's /copy would be another.
type Handler func(ctx context.Context, args json.RawMessage) (any, *MethodError)

// NamedResult is one response of a multi-response method: the response name
// (which for an implicit call is the IMPLICIT method's name, per §7.5's
// example showing "Email/set" under the submission call's id) and its
// arguments value.
...
// MultiHandler is a method that may produce several responses, in order, all
// under the calling invocation's id.
type MultiHandler func(ctx context.Context, args json.RawMessage) ([]NamedResult, *MethodError)
...
// RegisterMulti adds a multi-response method (see MultiHandler). Same
// contract and panics as Register.
func (r *Registry) RegisterMulti(name, capability string, h MultiHandler) {
	if name == "" || capability == "" || h == nil {
		panic("jmap: RegisterMulti requires a name, a capability and a handler")
	}
	if _, dup := r.methods[name]; dup {
		panic(fmt.Sprintf("jmap: method %q registered twice", name))
	}
	r.methods[name] = registeredMethod{capability: capability, multi: h}
}
...
func (e *Engine) Process(ctx context.Context, body []byte, sessionState string) (*Response, *RequestError) {
	...
	// The request's creation-id map (§3.3), seeded from the client's optional
	// createdIds argument and grown by every successful /set create. It rides
	// the context because the Handler signature must not change per feature —
	// and because a handler that does not create simply never touches it.
	created := NewCreationIDs(req.CreatedIDs)
	ctx = WithCreationIDs(ctx, created)
	...
	if req.CreatedIDs != nil {
		// §3.4: createdIds is "only returned if given in the request", and
		// then "MUST include all creation ids passed in the original
		// createdIds parameter" plus everything created since — which is
		// exactly the map's current contents.
		resp.CreatedIDs = created.Snapshot()
	}
	return resp, nil
}

// dispatchOne runs a single method call and returns its response invocations
// — one for an ordinary method, several for a MultiHandler (each under the
// same call id, §3.2), always at least one.
func (e *Engine) dispatchOne(ctx context.Context, call Invocation, using map[string]bool, prior []Invocation) []Invocation {
	...
	if !known {
		...
		// name.
		return single(NewMethodError(CodeUnknownMethod).Invocation(call.CallID))
	}
	...
	if !using[m.capability] {
		return single(NewMethodError(CodeUnknownMethod).
	...
	if merr != nil {
		return single(merr.Invocation(call.CallID))
	}

	var results []NamedResult
	if m.multi != nil {
		results, merr = e.safeCallMulti(ctx, call.Name, m.multi, args)
	} else {
		var result any
		result, merr = e.safeCall(ctx, call.Name, m.handler, args)
		results = []NamedResult{{Name: call.Name, Result: result}}
	}
	if merr != nil {
		return single(merr.Invocation(call.CallID))
	}
	if len(results) == 0 {
		// A multi handler that succeeded with nothing to say is a bug: §3.2
		// requires at least one response per call.
		e.log.Error("jmap: method produced no responses", "method", call.Name)
		return single(NewMethodError(CodeServerFail).
			WithDescription("method produced no responses").
			Invocation(call.CallID))
	}

	out := make([]Invocation, 0, len(results))
	for _, res := range results {
		raw, err := json.Marshal(res.Result)
		if err != nil {
			e.log.Error("jmap: marshaling method result failed",
				"method", call.Name, "response", res.Name, "error", err)
			return single(NewMethodError(CodeServerFail).
		...
		if !isJSONObject(raw) {
			...
			e.log.Error("jmap: method result is not a JSON object",
				"method", call.Name, "response", res.Name)
			return single(NewMethodError(CodeServerFail).
				WithDescription("method produced a non-object result").
				Invocation(call.CallID))
		}
		out = append(out, Invocation{Name: res.Name, Args: raw, CallID: call.CallID})
	}
	return out
}

// single wraps one invocation for dispatchOne's slice-shaped return.
func single(inv Invocation) []Invocation { return []Invocation{inv} }

// safeCallMulti is safeCall for multi-response methods: same panic
// containment, same serverFail degradation.
func (e *Engine) safeCallMulti(ctx context.Context, name string, h MultiHandler, args json.RawMessage) (results []NamedResult, merr *MethodError) {
	defer func() {
		if r := recover(); r != nil {
