...
- **A5 — Mecanismos de G3 que aplican YA** *(≈ 3 días)*: atajos por `event.code` [XS — verificado: hoy resolvemos por `event.key` y perdemos todo teclado no-latino], escape RFC 6901 de JSON Pointer [XS — obligatorio antes de C4], `RequestTimeoutError` jamás reintentado [test sobre nuestro cliente — reenviar un `EmailSubmission/set` es enviar el mail dos veces], separar create de destroy en borradores [test], `Message-ID` generado por el cliente + omitir `Cc` vacío [verificación del ensamblado W3, con test], los dos bypasses de DOMPurify documentados + normalización C0/CSS-escape en URLs [S], **rebuild de `srcDoc` al desbloquear imágenes pinneado por test** [XS — hoy ya se recompone por `useMemo` en `SecureHtmlBody.tsx`; el test evita que una refactorización lo rompa en silencio], **`coalesceRefresh` sobre el refresh por SSE** [S — nuestro fan-out tiene la misma forma que el que a Bulwark le blanqueó el árbol de carpetas].
